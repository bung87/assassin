...
https://github.com/gg/pingparser/blob/master/src/pingparser.py
"""
...
def parse(ping_output):
    ...
    fields:
        ...
        `jitter`: *float*; the standard deviation between round trip ping times
                    in milliseconds
    """
    matcher = re.compile(r'PING ([a-zA-Z0-9.\-]+) \(([0-9.]+)')
    group = _get_match_groups(ping_output, matcher)
    host = group[0]
    ip = group[1]
    ...
    return {'host': host, 'ip':ip,'sent': sent, 'received': received,
